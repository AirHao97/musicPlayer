    ...
    <div className='Body'>
        <div className="navList">
            ...
            <ul className='myLoveMusic'>
                <li><NavLink to='/content/findMusic'><h3>创建的歌单<span className='iconfont'>&#xe771;</span></h3></NavLink></li>
                <li><NavLink to='/index'><span className='iconfont'>&#xeca1;</span> 本地与下载</NavLink></li>
                {
                    playList.map(
                        (playListObj)=>{
                            return <li key={playListObj.id}><NavLink to={`/content/songsList?id=${playListObj.id}`} state={playListObj} >{playListObj.name}</NavLink></li>
                        }
                    )
                }
            </ul>
